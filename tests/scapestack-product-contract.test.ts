...
describe("Scapestack anti-bankstanding product contract", () => {
  it("keeps the core promise simple and OSRS-native", () => {
    ...
    expect(next).toContain("Pick one of these if the first trip is blocked or not what you feel like doing.");
    expect(next).toContain("const fallbackRecs = pick ? pick.alternatives.slice(0, 2) : [];");
