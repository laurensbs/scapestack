...
describe("Scapestack anti-bankstanding product contract", () => {
  it("keeps the core promise simple and OSRS-native", () => {
    ...
    expect(next).toContain("Pick a path");
    expect(next).toContain("Click an unlock, item or boss to see the exact trip.");
    expect(next).toContain("RouteCard");
    expect(next).toContain("data-route-card");
    expect(next).toContain("data-route-id-strip");
    expect(next).toContain("Worth it because");
