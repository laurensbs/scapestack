...
describe("hero intake copy and routing", () => {
  ...
  it("turns the homepage account-first when an RSN is already saved", () => {
    ...
    expect(source).toContain("setRememberedRuneliteCheckedAt(snapshot.runeliteCheckedAt);");
    ...
    expect(source).toContain('accountSnapshot?.runeliteDetail ?? "RuneLite later"');
    expect(source).toContain("const bankStatusLabel = accountSnapshot?.bankDetail");
    expect(source).toContain("const bankButtonLabel = accountSnapshot?.bankLabel");
    ...
    expect(source).toContain('`Vibe: ${accountSnapshot?.moodLabel ?? returningMood?.label ?? "Best now"}`');
    ...
    expect(source).toContain('text={`${accountSnapshot?.moodLabel ?? returningMood?.label ?? "Best now"} vibe`}');
    expect(source).not.toContain("UserRound");
    expect(source).toContain("Open today&apos;s trip");
    expect(source).toContain("{bankButtonLabel}");
    expect(source).toContain("accountSnapshot?.dpsHref");
    expect(source).toContain("accountSnapshot?.pluginHref");
