...
describe("plugin page copy constants", () => {
  it("keeps the player sync flow focused on the active account and one status", () => {
    ...
    expect(pageSource).toContain("How Scapestack and the plugin work together");
    expect(pageSource).toContain("What the plugin does");
    expect(pageSource).toContain("Connecting takes one button in the game and one in your browser.");
    expect(pageSource).toContain("What it sends");
