...
const state = vi.hoisted(() => ({
  ...
  page: { moments: [] as Array<Record<string, unknown>>, nextCursor: null as string | null },
...
beforeEach(() => {
  state.account = null;
  state.page = { moments: [], nextCursor: null };
...
describe("account timeline API", () => {
  ...
  it("returns no placeholder cards for an empty connected account", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ ok: true, moments: [], nextCursor: null, account: { rsn: "lauky" } });
  });
  ...
  it("returns a partial or returning page without inventing missing moments", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ moments: [{ title: "Gained 180k Slayer XP" }], nextCursor: "valid-cursor" });
  });
