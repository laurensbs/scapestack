...
describe("/next confidence UI copy", () => {
  ...
  it("renders every recommendation as an actionable OSRS session card", () => {
    ...
    expect(source).toContain('label: accountMode.type === "ultimate" ? "Carry" : "Bank"');
  ...
  it("starts the result page with one plan instead of setup panels", () => {
    ...
    expect(source).toContain("Pick a path");
    expect(source).toContain("Click an unlock, item or boss to see the exact trip.");
    expect(source).toContain("function RouteCard");
    expect(source).toContain('data-route-card="true"');
    expect(source).toContain("aria-expanded={expanded}");
    expect(source).toContain("routeCardPositionLabel");
    ...
    expect(source).toContain('data-route-id-strip="true"');
    expect(source).toContain("`id:${sprite.itemId}`");
    expect(source).toContain("`boss:${sprite.slug}`");
    ...
    expect(source).toContain("routeWorthValue");
    expect(source).toContain('label="Do"');
    expect(source).toContain('label="Bring/check"');
    expect(source).toContain('label="Missing"');
    ...
    expect(source).toContain('label="Stop when"');
    expect(source).toContain('label="Worth it because"');
  ...
  it("surfaces account mode as a player-facing decision signal", () => {
    ...
    expect(source).toContain("accountModeVisual(accountMode.type, accountMode.confidence)");
  ...
  it("turns the main pick into a compact trip builder when gear exists", () => {
    ...
    expect(source).toContain("Show exact items");
    expect(source).toContain("Show banked skilling stack");
    expect(source).toContain("Bring list, teleport and Bank Tag.");
