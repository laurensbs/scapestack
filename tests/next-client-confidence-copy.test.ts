...
describe("/next confidence UI copy", () => {
  ...
  it("starts the result page with one plan instead of setup panels", () => {
    ...
    expect(source).not.toContain("Best now");
    ...
    expect(source).not.toContain("Choose a session instead");
    expect(source).not.toContain("Want a different kind of trip?");
    expect(source).not.toContain("INTAKE_SESSION_CHOICES");
    expect(source).not.toContain("What do you feel like doing?");
    expect(source).not.toContain("Plan best route");
    expect(source).not.toContain("setShowRoutePicker(true)");
    expect(source).not.toContain("runWithRoute(choice)");
    expect(source).not.toContain("Show ${nextRouteLabel.name}");
    ...
    expect(source).not.toContain("setSelectedRouteLens(choice.routeLens)");
    ...
    expect(source).not.toContain("function SessionMoodGrid");
    expect(source).not.toContain('data-session-mood-grid="true"');
    expect(source).not.toContain("SESSION_MOOD_GRID_CHOICES");
    expect(source).not.toContain("What are you in the mood for?");
    expect(source).not.toContain("Same mood, different route.");
    expect(source).not.toContain('label: "Surprise me"');
    // "vibe" is on the never-list in the design system's voice section.
    expect(source).not.toContain("vibe");
    expect(source).not.toContain("onSurprise={moveToAnotherPlan}");
    ...
    expect(source).toContain("Closest unlock");
    expect(source).toContain("Next unlock path");
    expect(source).toContain("Your routes");
    expect(source).toContain("Unlock paths");
    expect(source).not.toContain("More unlock moves");
    expect(source).not.toContain("Unlock gaps");
    expect(source).not.toContain("Route blockers worth checking");
    expect(source).toContain("A tick is done. The marked node is next.");
    ...
    expect(source).toContain("<UnlockRoutePath");
    ...
    expect(source).toContain("route.pathNodes");
  ...
  it("gives hidden recommendations an immediate undo path", () => {
    ...
    expect(source).not.toContain("Want AFK");
    expect(source).not.toContain('applySessionIntent("cash", 60)');
    expect(source).not.toContain('applySessionIntent("afk", 60)');
    expect(source).not.toContain("markLastSuppressedTooHard");
    expect(source).toContain("hidePlayerPlanRecommendation");
    expect(source).toContain("recommendation: rec");
  ...
  it("lets players mark recommendations done without calling them irrelevant", () => {
    ...
    expect(source).not.toContain("Chill now");
    expect(source).not.toContain('applySessionIntent("chill", 30, "completion")');
    expect(source).toContain("Undo done");
    expect(source).toContain("const moveToAnotherPlan = () =>");
    expect(source).not.toContain("const moveToChillPlan = () =>");
  ...
  it("lets bank-handoff players run /next without typing an RSN", () => {
    ...
    expect(source).toContain('routeLens: "smart"');
    expect(source).toContain("mood: DEFAULT_MOOD");
    expect(source).toContain("minutes: DEFAULT_TIME");
