...
describe("global header navigation", () => {
  it("marks Scapestack brand and primary tools with explicit navigation labels", () => {
    ...
    expect(source).toContain("setBankDetail(snapshot.bankDetail)");
    expect(source).toContain("setRuneliteDetail(snapshot.runeliteDetail)");
    expect(source).toContain("const bankFreshness = bankDetail;");
