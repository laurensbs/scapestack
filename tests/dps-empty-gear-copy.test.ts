...
describe("DPS empty gear copy", () => {
  it("does not imply DPS is calculated when the bank has zero weapons", () => {
    ...
    expect(source).toContain("Add setup for ${boss.name}");
    ...
    expect(source).toContain("Add setup here");
    ...
    expect(source).toContain("RuneLite skips finished account stuff, but boss checks still need your setup.");
    expect(source).toContain("This boss came from Task Check. Add setup before buying supplies or trusting upgrades.");
    expect(source).toContain("Add setup before using boss checks, upgrades or setup links.");
