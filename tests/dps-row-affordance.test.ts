...
describe("DPS boss row affordance", () => {
  ...
  it("renders bosses as explicit clickable cards instead of dashboard rows", () => {
    ...
    expect(source).toContain("function bossCardReason");
    expect(source).toContain("Best for this bank");
    expect(source).toContain('useState<SortKey>("fit")');
    expect(source).not.toContain("killPace");
    expect(source).not.toContain("trip items");
    expect(source).not.toContain("Est. {formatRateRange(profitEstimate.grossGpPerHour.range, formatGp)}/hr");
  ...
  it("starts DPS as a boss browser instead of a dashboard verdict", () => {
    ...
    expect(source).toContain('case "fit":      sorted.sort((a, b) => bossTripFitScore(b) - bossTripFitScore(a)); break;');
    ...
    expect(source).toContain("Tools and supplies checked from your bank");
