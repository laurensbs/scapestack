...
**useful** for a real player. This doc nails three concrete archetypes
...
2. **A diary that fits their stats** — Karamja Medium / Falador Medium
   / Varrock Medium — they've passively unlocked one and don't know it.
...
3. **If a scenario fails** the eyeball test, that's the next thing to
   fix — not the next feature. The bar for "shippable" is all three
...
Three is enough to catch the worst regressions across the spectrum
(low → mid → maxed) without bogging down. If we find a category of
account the engine handles badly that isn't covered here (HCIM-no-prayer,
F2P-only, pure), add a fourth. Don't pre-emptively pad the list.
